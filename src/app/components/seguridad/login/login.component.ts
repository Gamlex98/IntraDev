      ...
      this.serviceSeguridad.Login(datos).subscribe({
        next: (data: DatosSesionModel) => {
          ...
          if (data.tk) {
            ...
            //Garantice un tiempo para ver la ventana emergente antes de recargar la página y se vea el menú 
            setTimeout(() => {
              //Recargue la página y se vea el menú
              window.location.reload();  //El location.reload solo se debe usar en casos muy controlados
            }, 500);  //porque detiene TODOS los procesos
